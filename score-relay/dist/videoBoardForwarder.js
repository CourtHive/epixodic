...
export function configureVideoBoardForwarder(rawTarget) {
    if (!rawTarget) {
        target = null;
        return;
    }
...
export function forwardVideoBoardPayload(payload) {
    if (!target)
        return;
    const data = Buffer.from(JSON.stringify(payload));
    socket.send(data, target.port, target.host, (err) => {
        if (err)
            console.warn(`[videoBoardForwarder] send failed: ${err.message}`);
    });
}
// Test/cleanup helper.
export function shutdownVideoBoardForwarder() {
    target = null;
    socket.close();
}
